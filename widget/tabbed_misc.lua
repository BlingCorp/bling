...
function module.titlebar_indicator(c)
    ...
    awesome.connect_signal("bling::tabbed::update", function(group, widget_template)
        local focused = group.clients[group.focused_idx]
        if tbl_contains(group.clients, c) then
            tabbed_icons:reset()
            for idx, _c in ipairs(group.clients) do

                -- I need to refactor most of this to use theme vars...
                ...
                for _, w in ipairs(widget:get_children_by_id("icon_role")) do
                    w.client = _c -- for awful.widget.clienticon
                    w.image = _c.icon -- for wibox.widget.imagebox
                end
